const createTrackCard = (track) => {
    return `
    <article class="track-card group rounded-2x1 border-white/5 bg-zinc-900/80 p-3 transition duration-300 hover:-translate-y-1 hover:border-violet-500/40">
        <div class="relative overflow-hidden rounded-xl">
            ...
            <button class="play-track absolute buttom-3 right-4 grid h-11 w-11 place-items-center rounded-full bg-white text-black opacity-0 shadow-xl transition group-hover:opacity-100 focus:opacity-100"
                data-track-id="${track.id}"
                arial-label="Seleccionar ${track.title}">
                play
            </button>
...
const createRecentlyPlayedRow = (track) => {
    return ` 
        <article class="grid grid-cols-[auto_1fr_auto_auto] items-center gap-3 rouned-2xl px-3 py-2 transition hover:bg-zinc-900">
            <img 
                src="${track.cover}" 
                alt="Portada de ${track.title}" 
            ...
            <div class="min-w-0">
                <h3 class="truncate text-sm font-semibold">${track.title}</h3>
            ...
            <button class="play-track grid w-9 h-9 place-items-center rounded-full 
